...
export function ActivityPanel({
  ...
  if (isLoading && !data.length)
    return <PanelLoading label="Loading activity trends" />;
          ...
          return (
            ...
            >
              ...
              <p className="mt-1 text-[10px] font-semibold text-[#aaa5a1]">
                {formatActivityDate(activePeriod)}
              </p>
            </article>
          );
        })}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-bold text-[#1b1c19]">
              Showing data for {formatActivityDate(selectedPeriod)}
            </p>
          </div>
        </div>
    ...
    <section className="overflow-hidden rounded-[28px] border border-black/5 bg-white shadow-[0_14px_40px_rgba(49,20,133,0.05)]">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[760px] border-collapse text-left">
          <thead className="bg-[#fbf9f4] text-[11px] font-extrabold uppercase tracking-[0.08em] text-[#797583]">
            <tr>
              <th className="w-[24%] px-5 py-3 sm:px-6" scope="col">
                Period
              </th>
              <th className="px-3 py-3" scope="col">New</th>
              <th className="px-3 py-3" scope="col">Active</th>
              <th className="px-3 py-3" scope="col">Sessions</th>
              <th className="px-3 py-3" scope="col">Reviews</th>
              <th className="px-3 py-3" scope="col">Accuracy</th>
            </tr>
          </thead>
          <tbody>
            {data.map((item) => (
              <tr
                aria-current={
                  item.period === selectedPeriod ? "date" : undefined
                }
                className={`border-t text-sm font-semibold text-[#1b1c19] transition ${
                ...
                key={item.period}
              >
                <td className="px-5 py-4 font-bold text-[#5f5e5e] sm:px-6">
                  {formatActivityDate(item.period)}
                </td>
                <td className="px-3 py-4">{formatNumber(item.newUsers)}</td>
                <td className="px-3 py-4">{formatNumber(item.activeUsers)}</td>
                <td className="px-3 py-4">{formatNumber(item.sessions)}</td>
                <td className="px-3 py-4">{formatNumber(item.reviews)}</td>
