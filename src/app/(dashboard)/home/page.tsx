...
export default function DashboardHome() {
  ...
  const activeProgress = activeSummary?.total
    ? Math.round((activeSummary.mastered / activeSummary.total) * 100)
    : 0;
  ...
  const reviewedToday = sessions
    .filter(
      (session) => new Date(session.startedAt).toDateString() === todayKey,
    )
    .reduce(
      (sum, session) => sum + session.stats.correct + session.stats.wrong,
      0,
    );
  const studyDays = new Set(
    sessions
      .filter((session) => session.stats.correct + session.stats.wrong > 0)
  ...
  const week = Array.from({ length: 7 }, (_, index) => {
    ...
    return {
      ...
      done: studyDays.has(date.toDateString()),
    };
  });
  const goal = 20;
      ...
      <div className="mx-auto w-full max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8">
        <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-normal text-[#1b1c19] sm:text-4xl">
        ...
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-w-0 space-y-10">
            <section>
              ...
              {activeDeck ? (
                <article className="relative overflow-hidden rounded-[30px] border border-[#cfc4f5] bg-[#e6deff] p-6 shadow-[0_18px_50px_rgba(49,20,133,0.09)] sm:p-8">
                  ...
                  <div className="relative z-10 max-w-[650px]">
                    <div className="mb-7 flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-s font-bold uppercase tracking-[0.13em] text-[#614db7]">
                          Current deck
                        </p>
                        <h3 className="mt-1 text-3xl font-semibold tracking-normal text-[#311485] sm:text-3xl">
                          {activeDeck.title}
                        </h3>
                      </div>
                    </div>
                    <p className="max-w-[560px] text-sm leading-6 text-[#4e3d88] sm:text-base">
                      {activeDeck.description ||
                        "Your next review is ready whenever you are."}
                    </p>
                ...
                <div className="rounded-[30px] border border-dashed border-[#cabeff] bg-[#f6f2ff] p-8 text-center">
                  ...
                  <p className="mt-2 text-sm text-[#777474]">
                    Your real decks will appear here after creation.
                  </p>
                  <Link
                    className="mt-5 inline-flex rounded-full bg-[#614db7] px-5 py-3 text-sm font-bold text-white"
                    href="/my-library"
                  >
                    Open library
                  </Link>
            ...
            <section>
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[#777474]">
                    Jump back into your latest material.
                  </p>
                </div>
                <Link
                ...
                {decks.slice(0, 4).map((deck, index) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <span
                        className={`flex h-11 w-11 items-center justify-center rounded-2xl ${index % 2 === 0 ? "bg-[#ffd9e4] text-[#7b3451]" : "bg-[#d7f2e3] text-[#276345]"}`}
                      >
                        <Layers3 className="h-5 w-5" />
                      </span>
                      <ChevronRight className="h-5 w-5 text-[#aaa5a0] transition group-hover:translate-x-1 group-hover:text-[#614db7]" />
                    </div>
                    <h3 className="mt-5 text-lg font-semibold tracking-normal text-[#1b1c19]">
                      {deck.title}
                    </h3>
                    <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#777474]">
                      {deck.description ||
                        "A focused set of cards ready for active recall."}
                    </p>
              ...
              {!decks.length && (
                <p className="rounded-[22px] bg-white p-5 text-sm text-[#777474]">
                  No recent decks yet.
                </p>
              )}
            </section>
              ...
              <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                <div className="max-w-[590px]">
                  ...
                  <h2 className="mt-3 text-2xl font-semibold sm:text-3xl">
                    Turn rough notes into a clean study deck.
                  </h2>
                  <p className="mt-3 text-sm leading-6 text-white/65 sm:text-base">
                    Start with manual cards today. AI generation will slot into
                    this same workflow when the backend is ready.
                  </p>
                </div>
                <Link
                  className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#f5d547] px-6 py-3 text-sm font-extrabold text-[#493600] transition hover:-translate-y-0.5 hover:bg-[#ffe36a]"
                  href="/flashcards"
                >
          ...
          <aside className="space-y-5">
            <section className="rounded-[26px] border border-black/5 bg-white p-5 shadow-[0_12px_36px_rgba(27,28,25,0.05)]">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-extrabold uppercase tracking-[0.13em] text-[#9a6900]">
                    Daily streak
                  </p>
                  <p className="mt-2 text-3xl font-bold">{streak} days</p>
                </div>
                <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#fff1b3] text-[#b06f00]">
              ...
              <div className="mt-6 grid grid-cols-7 gap-1.5">
                {week.map((day, index) => (
                  <div className="text-center" key={index}>
                    <span
                      className={`mx-auto block h-7 w-7 rounded-full ${day.done ? "bg-[#f5d547]" : "bg-[#f0ece7]"}`}
                    />
                    <span className="mt-2 block text-[10px] font-bold text-[#9a9692]">
                      {day.label}
                    </span>
                  </div>
                ))}
            ...
            <section className="rounded-[26px] border border-black/5 bg-white p-5 shadow-[0_12px_36px_rgba(27,28,25,0.05)]">
              ...
              <div className="mt-3 flex justify-between text-xs font-bold text-[#777474]">
                <span>{reviewedToday} completed</span>
                <span>{Math.max(0, goal - reviewedToday)} remaining</span>
              </div>
            </section>

            <section className="rounded-[26px] bg-[#311485] p-5 text-white shadow-[0_16px_40px_rgba(49,20,133,0.18)]">
              <div className="flex items-start justify-between gap-4">
                <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/10 text-[#f5d547]">
                  <Trophy className="h-5 w-5" />
                </span>
                <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-[#cabeff]">
                  {user?.totalPoints ?? 0} XP
                </span>
              </div>
              <h2 className="mt-6 text-xl font-semibold">Your momentum</h2>
              <p className="mt-2 text-sm leading-6 text-white/65">
                One short review today keeps your learning rhythm moving.
              </p>
              <Link
                className="mt-5 inline-flex items-center gap-2 text-sm font-extrabold text-[#f5d547]"
                href="/my-library"
              >
                Choose a deck <ArrowRight className="h-4 w-4" />
              </Link>
            </section>

            <section className="rounded-[26px] border border-black/5 bg-[#f2eefe] p-5">
              <div className="flex items-center gap-3 text-[#614db7]">
                <BrainCircuit className="h-5 w-5" />
                <p className="text-xs font-extrabold uppercase tracking-[0.13em]">
                  Quick tip
                </p>
              </div>
              <p className="mt-4 text-sm font-semibold leading-6 text-[#4e3d88]">
                Short, repeated sessions usually beat one long review. Aim for
                ten focused minutes.
              </p>
              <div className="mt-4 flex items-center gap-2 text-xs font-bold text-[#7566aa]">
